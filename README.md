...
a) Get an `.mp3` file
b) Get a good transcription of the audio, with precise timestamps
c) Put them in the right place in our data folder so that the code knows wwhere to find it
d) run `data_recording/perceived_speech.py`, and use the GUI to click `play`.
